...
public class UnconsumedExceptionThrowerLink
     ...
     *
     * @param types the types of events to ignore.
     */
    public UnconsumedExceptionThrowerLink(Class<?>... types) {
        super(matches(types).or(matchesAutoConsumable()));
    }

    @Override
    public void eventHandler(Event event) {
        if (!event.isConsumed() && !applyCondition(event))
