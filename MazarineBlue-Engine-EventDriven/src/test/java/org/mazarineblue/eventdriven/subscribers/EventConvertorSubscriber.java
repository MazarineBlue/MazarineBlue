 ...
 */
package org.mazarineblue.eventdriven.subscribers;
 ...
 */
public class EventConvertorSubscriber
        extends ReflectionSubscriber<Event> {

    private final Event replacement;

    public EventConvertorSubscriber(Event replacement) {
     ...
     */
    @EventHandler
    public void eventHandler(InvokerEvent event) {
        if (event.equals(replacement))
            return;
        event.setConsumed(true);
        event.invoker().publish(replacement);
    }
}
