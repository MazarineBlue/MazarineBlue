...
import org.mazarineblue.eventbus.util.SubscriberSpy;
...
public class DefaultFeedExecutorTest {
    ...
    public void setup() {
        ...
        feedExecutorListenerSpy = new FeedExecutorListenerSpy(matchesNoneAutoConsumable());
        publisherListenerSpy = new PublisherListenerSpy(matchesNoneAutoConsumable());
    ...
    public class TestFileFeed {
        ...
        public void execute_CorruptFeed_ExceptionReported() {
            PluginLoader.getInstance().injectPlugin(new MemoryFeedPlugin()
                    .addSheet("Main", () -> new ExceptionThrowingFeed()));
            fs.mkfile(FEED_FILE, "");
    ...
    public class TestListeners {
        ...
        public void execute_InterpreterListener() {
            ...
            assertEquals(1, interpreterListenerSpy.countLinks());
            assertFalse(fs.exists(LOG_FILE));
        }
    ...
    public class TestWithSubscriber {
        ...
        public void setup() {
            ...
            factory.setInterpreterListener(() -> interpreterListenerSpy);
            factory.addSubscriber(SubscriberFactory.newInstance(Event.class, null, subscriberSpy));
            factory.addLinkAfterEventBus(() -> linkAfterBus);
            factory.addLink(() -> l);
            factory.addLink(() -> linkBeforeBus);
        ...
        public void test() {
            ...
            assertEquals(1, interpreterListenerSpy.countOpeningFeed());
            assertEquals(1, interpreterListenerSpy.countClosingFeed());
            assertEquals(3, interpreterListenerSpy.countStartingEvents());
            assertEquals(0, interpreterListenerSpy.countExceptions());
            assertEquals(3, interpreterListenerSpy.countEndingEvents());
            assertEquals(4, interpreterListenerSpy.countLinks());

            assertEquals(2, subscriberSpy.getCount());
            assertEquals(3, linkBeforeBus.size());
            assertEquals(2, linkAfterBus.size());
            assertTrue(fs.exists(LOG_FILE));
        }
    }
}
