...
public class LibraryService {
    ...
    public static List<Library> getLibraries(String namespace) {
        return PluginLoader.getInstance().getPlugins(LibraryPlugin.class).stream()
                ...
                .flatMap(plugin -> createLibraryStream(plugin, namespace))
                .collect(Collectors.toList());
    }

    private static Stream<Library> createLibraryStream(LibraryPlugin plugin, String namespace) {
        try {
            return plugin.getLibraries(namespace).stream();
        } catch (RuntimeException ex) {
            throw new LibraryPluginException(namespace);
        }
    }
