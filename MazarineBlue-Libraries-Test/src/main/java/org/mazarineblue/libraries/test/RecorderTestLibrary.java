...
import static org.mazarineblue.keyworddriven.events.InstructionLineEvent.anyKeywordMatch;
...
import org.mazarineblue.links.RecorderLink;

public class RecorderTestLibrary
        extends AbstractTestLibrary {

    private static final Predicate<Event> stopRecordingCondition = anyKeywordMatch("Setup", "Teardown", "Suite", "Test", "End test system");

    private final TestSystem system;
    private final Suite globalSuite;
    private Suite activeSuite;
    private Test activeTest;
    private RecorderLink setup;
    private RecorderLink test;
    private RecorderLink teardown;
    ...
    private void startSetupRecording(Invoker invoker) {
        ...
        setup = new RecorderLink(invoker, stopRecordingCondition);
        invoker.interpreter().addLink(setup);
    }
    ...
    private void startTeardownRecording(Invoker invoker) {
        ...
        teardown = new RecorderLink(invoker, stopRecordingCondition);
        invoker.interpreter().addLink(teardown);
    }
    ...
    private void startTestRecording(Invoker invoker, String name) {
        ...
        activeTest = t;
        test = new RecorderLink(invoker, stopRecordingCondition);
        invoker.interpreter().addLink(test);
    }
}
