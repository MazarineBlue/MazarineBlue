...
/**
 * An {@code AbstractSetupLibrary} is an {@code AbstractBrowserManagerLibrary} that are able to setup, start and stop
 * browsers.
 * 
 * @author Alex de Kruijff <alex.de.kruijff@MazarineBlue.org>
 */
public abstract class AbstractSetupLibrary
        extends AbstractBrowserManagerLibrary {

    public AbstractSetupLibrary(BrowserRegistry manager) {
        super(manager);
    }

    @Override
    protected void startBrowser(Invoker invoker, Browser browser) {
        super.startBrowser(invoker, browser);
        invoker.publish(new RemoveLibraryEvent(this));
        invoker.publish(new AddLibraryEvent(new BrowserLibrary(getManager())));
    }
}
