...
public class Timestamp {
    ...
    static private final String defaultDateFormat = "yyyy-MM-dd HH:mm:ss X";
    private static final Map<String, Timestamp> map = new HashMap<>(4);

    public static Timestamp getDefaultInstance() {
        return getDefaultInstance(defaultDateFormat);
    }
    ...
    public static Timestamp getDefaultInstance(String format) {
        ...
        synchronized (map) {
            if (map.containsKey(format) == false) {
                Timestamp timestamp = new Timestamp(format);
                map.put(format, timestamp);
                return timestamp;
            }
            return map.get(format);
        }
